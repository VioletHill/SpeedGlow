...
@implementation ByymMidiumLevelLayer
{
    bool isLock;
}
...
-(void) unlockLevel
{
    ...
    if ([Setting sharedSetting].isNeedEffect)
    {
        int sun=[[UserData sharedUserData] getSunByScene:kBYYM andLevel:kMIDIUM];
        playEffectAction=[CCSequence actions:
                      ...
                      [CCDelayTime actionWithDuration:1.5],[CCCallFuncND actionWithTarget:self selector:@selector(playNum:data:) data:(void*)ByymMidiumSunNum],
...
-(void) onEnterLayer
{
    if ([[UserData sharedUserData] getIsUnlockAtScene:kBYYM andLevel:kMIDIUM])
    {
        [self unlockLevel];
        isLock=false;
    }
