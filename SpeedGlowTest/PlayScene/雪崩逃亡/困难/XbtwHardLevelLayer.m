...
@implementation XbtwHardLevelLayer
{
    bool isLock;
}
...
-(void) unlockLevel
{
    if ([Setting sharedSetting].isNeedEffect)
    {
        int sun=[[UserData sharedUserData] getSunByScene:kXBTW andLevel:kHARD];
        playEffectAction=[CCSequence actions:
                      ...
                      [CCDelayTime actionWithDuration:1.5],[CCCallFuncND actionWithTarget:self selector:@selector(playNum:data:) data:(void*)XbtwHardSunNum],
...
-(void) onEnterLayer
{
    ...
    if ([[UserData sharedUserData] getIsUnlockAtScene:kXBTW andLevel:kHARD])
    {
        [self unlockLevel];
        isLock=false;
    }
