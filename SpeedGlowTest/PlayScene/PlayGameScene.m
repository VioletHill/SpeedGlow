...
{
    CCSprite* raceTimeMinute[5];        //0~5
    
    CCSprite* raceTimeSecond2[10];       //0~10  个位
    CCSprite* raceTimeSecond1[10];      //0~10   十位
    
    CCSprite* remainTimeMinute[5];      //0~5
    CCSprite* remainTimeSecond[10];     //0~10
    
    int raceTimeInGame;
    int raceTimeSecondIndex;
    int raceTimeMinuteIndex;
...
-(void) loadYlfcSceneMap
{
    ...
    {
        NSLog(@"%d %d %f",i,obstacle[i],obstacleDistance[i]);
    }
}
...
-(void) startGo:(id)pSender
{
    ...
    //初始化时间
    raceTimeSecondIndex=0;
    raceTimeInGame=0;
    raceTimeMinuteIndex=0;
...
{
    //raceTime title
    CCSprite* raceTime=[CCSprite spriteWithFile:@"RaceTime.png"];
    raceTime.anchorPoint=ccp(1,1);
    raceTime.position=ccp(screenSize.width,screenSize.height);
    [self addChild:raceTime];
    
    //time
    for (int i=0; i<10; i++)
    {
        raceTimeSecond2[i]=[CCSprite spriteWithFile:[NSMutableString stringWithFormat:(@"small%d.png"),i]];
        raceTimeSecond2[i].anchorPoint=ccp(1,1);
        raceTimeSecond2[i].position=ccp(screenSize.width,screenSize.height-raceTime.contentSize.height);
        [self addChild:raceTimeSecond2[i]];
        [raceTimeSecond2[i] setVisible:false];
        
        raceTimeSecond1[i]=[CCSprite spriteWithFile:[NSMutableString stringWithFormat:(@"small%d.png"),i]];
        raceTimeSecond1[i].anchorPoint=ccp(1,1);
        raceTimeSecond1[i].position=ccp(screenSize.width-raceTimeSecond1[i].contentSize.width,screenSize.height-raceTime.contentSize.height);
        [self addChild:raceTimeSecond1[i]];
        [raceTimeSecond1[i] setVisible:false];
    }

    [raceTimeSecond1[0] setVisible:true];
    [raceTimeSecond2[0] setVisible:true];
    
    //////////////////////////////////
    for (int i=0; i<5; i++)
    {
        raceTimeMinute[i]=[CCSprite spriteWithFile:[NSMutableString stringWithFormat:(@"small%d.png"),i]];
        raceTimeMinute[i].anchorPoint=ccp(1,1);
        // raceTimeMinute[i]
...
-(id) init
{
    if (self=[super init])
    {
        ...
        CCMenu* normalMenu=[CCMenu menuWithItems:normalItem, nil];
        normalItem.anchorPoint=ccp(0,0);
        [normalMenu setAnchorPoint:ccp(0,0)];
        normalMenu.position=ccp(0,0);
        [self addChild:normalMenu];
        ...
        CCMenu* speedUpMenu=[CCMenu menuWithItems:speedUpItem, nil];
...
{
    [raceTimeSecond1[raceTimeSecondIndex/10] setVisible:false];
    [raceTimeSecond2[raceTimeSecondIndex%10] setVisible:false];
    
    raceTimeSecondIndex++;
    raceTimeInGame++;
    
    if (raceTimeSecondIndex==60) raceTimeSecondIndex=0;
    
    [raceTimeSecond1[raceTimeSecondIndex/10] setVisible:true];
    [raceTimeSecond2[raceTimeSecondIndex%10] setVisible:true];
}
